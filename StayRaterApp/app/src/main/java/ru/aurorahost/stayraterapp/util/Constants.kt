package ru.aurorahost.stayraterapp.util

object Constants {
    const val STAY_RATER_DB = "stay_rater_db"
    const val DETAILS_ARGUMENT_KEY = "hotelId"
    const val HOTEL_DATABASE_TABLE = "hotel_table"
    const val HOTEL_REMOTE_KEYS_DATABASE_TABLE = "hotel_remote_keys_table"

    const val PREFERENCES_NAME = "stay_rater_preferences"
    const val PREFERENCES_KEY = "on_boarding_completed"

    const val ON_BOARDING_PAGE_COUNT = 3
    const val LAST_ON_BOARDING_PAGE = 2

    const val BASE_URL = "http://10.0.2.2:8080"

    const val ITEMS_PER_PAGE = 3
}